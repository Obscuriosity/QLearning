'''
QL_256_Explore
    Robot Q Learning experiment with 256 states and 3 actions. 768 Q Table entries!
    States are 0 or 1 on eight sonar to imply objects are within 15cm
    ...
    will try large negative reward for crash, works quite well but agent will sit in front of object swaying left and right.
    Will try the following to assign reward based on state/environment to encourage the agent to improve
    state by moving? example below -
...
def SONAR(position): # Retrieve the state of individual Sonar
    global dataList, SHORT_DISTANCE
    # Eight Sonar Script
    if position == 0:
        distance = dataList[3]
...
def getState(): # Returns state of the percieved world as a list i,e, distances from sonars and speed of wheels
    ...
    newState = [S1, S2, S3, S4, S5, S6, S7, S8]
...
def getReward():
    global crashed, states, s, a, REWARD_LIST
    r = 0
